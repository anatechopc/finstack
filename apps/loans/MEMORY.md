...
- **G12 penalties golden set (2026-09-08, branch `test/penalties-golden-g12` stacked on this one):** `test/services/penalty_golden_test.dart`, 8 hand-derived scenarios walking company `defaultPenalties` → product copy-on-create → `Loan.create` snapshot → `resolveLateness`/`applyLateness`. Pins both owner-required outcomes: defaults imposed → 5 days late on 2,500 charges 225; no company defaults → still `paid_late`, 0, nothing to penalize (and nothing to waive, no reason needed). Also cadence rounding, the open-term balance base, and that the ONLY trigger is a collection after the due date (no grace/threshold/cap exists). Mutation check: `(daysLate / 30).ceil()` → `/ 31` in loooans_helpers' `penalty.dart` fails P5.
