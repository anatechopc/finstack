...
/// bank_details firestore database service
final class BankDetailsFirestoreService
    extends BaseFirestoreService<BankDetailsEntity> {
  BankDetailsFirestoreService() {
  ...
  @override
  String get collectionName => 'bank_details';

  @override
  Future<BankDetailsEntity> add({required BankDetailsEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<BankDetailsEntity> delete({required BankDetailsEntity data}) async {
  ...
  @override
  Future<BankDetailsEntity> get(
      {required String id, bool isCache = false,}) async {
    final doc = await root
        .doc(id)
        .get(!isCache ? null : const GetOptions(source: Source.cache));

    return BankDetailsEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<BankDetailsEntity> update({required BankDetailsEntity data}) async {
  ...
  @override
  Future<List<BankDetailsEntity>> load({
    ...
    return data.docs
        .map(
          (doc) =>
              BankDetailsEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
      ...
      controller.addStream(
        query
          ...
          return snapshot.docs
              .map(
                (doc) => BankDetailsEntity.fromJson(
  ...
  @override
  Stream<List<BankDetailsEntity>> get dataStream => controller.stream;
}
