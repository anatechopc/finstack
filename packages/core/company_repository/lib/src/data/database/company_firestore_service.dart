...
final class CompanyFirestoreService
    extends BaseFirestoreService<CompanyEntity> {
  ...
  @override
  Future<CompanyEntity> add({required CompanyEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<CompanyEntity> delete({required CompanyEntity data}) async {
  ...
  @override
  Future<CompanyEntity> get({required String id, bool isCache = false}) async {
    final doc = await root
        .doc(id)
        .get(!isCache ? null : const GetOptions(source: Source.cache));

    return CompanyEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<CompanyEntity> update({required CompanyEntity data}) async {
  ...
  @override
  Future<List<CompanyEntity>> load({
    ...
    return data.docs
        .map(
          (doc) => CompanyEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
  ...
  Future<void> loadNext({
    ...
    if (!switchStream) {
      ...
      final mappedData = data.docs
          .map(
            (doc) =>
                CompanyEntity.fromJson(doc.data()! as Map<String, dynamic>),
          )
          .toList();
  ...
  @override
  Stream<List<CompanyEntity>> get dataStream => controller.stream;
}
