...
final class LoanScheduleFirestoreService
    extends BaseFirestoreService<LoanScheduleEntity> {
  LoanScheduleFirestoreService() {
  ...
  @override
  Future<LoanScheduleEntity> add({required LoanScheduleEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<LoanScheduleEntity> delete({required LoanScheduleEntity data}) async {
  ...
  Future<LoanScheduleEntity> get({
    ...
    return LoanScheduleEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<LoanScheduleEntity> update({required LoanScheduleEntity data}) async {
  ...
  Future<List<LoanScheduleEntity>> allByLoanId({
    required String loanId,
    required bool onlyPaid,
  }) async {
    var query = root.where('loan_id', isEqualTo: loanId);
    ...
    final doc = await query.orderBy('due_at').get();
    final schedules = doc.docs
        .map((e) => LoanScheduleEntity.fromJson(e.data()! as Map<String, dynamic>))
        .toList();
  ...
  @override
  Future<List<LoanScheduleEntity>> load({
    ...
    return data.docs
        .map(
          (doc) =>
              LoanScheduleEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
  ...
  Future<void> loadNext({
    ...
    if (!switchStream) {
      ...
      final mappedData = data.docs
          .map(
            (doc) => LoanScheduleEntity.fromJson(
  ...
  @override
  Stream<List<LoanScheduleEntity>> get dataStream => controller.stream;
}
