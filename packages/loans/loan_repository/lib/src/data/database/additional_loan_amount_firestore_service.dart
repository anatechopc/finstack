...
final class AdditionalLoanAmountFirestoreService extends BaseFirestoreService<AdditionalLoanAmountEntity> {
  ...
  @override
  String get collectionName => 'additional_loan_amounts';

  @override
  Future<AdditionalLoanAmountEntity> add({required AdditionalLoanAmountEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<AdditionalLoanAmountEntity> delete({required AdditionalLoanAmountEntity data}) async {
  ...
  @override
  Future<AdditionalLoanAmountEntity> get({
    required String id,
    bool isCache = false,
  }) async {
    ...
    return AdditionalLoanAmountEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<AdditionalLoanAmountEntity> update({required AdditionalLoanAmountEntity data}) async {
  ...
  @override
  Future<List<AdditionalLoanAmountEntity>> load({
    ...
    return data.docs
        .map(
          (doc) => AdditionalLoanAmountEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
      ...
      controller.addStream(
        query.snapshots().map((snapshot) {
          return snapshot.docs
              .map(
                (doc) =>
                    AdditionalLoanAmountEntity.fromJson(doc.data()! as Map<String, dynamic>),
              )
              .toList();
        }),
      ),
  ...
  @override
  Stream<List<AdditionalLoanAmountEntity>> get dataStream => controller.stream;
}
