...
final class SettingsFirestoreService
    extends BaseFirestoreService<SettingsEntity> {
  ...
  @override
  Future<SettingsEntity> add({required SettingsEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<SettingsEntity> delete({required SettingsEntity data}) async {
  ...
  @override
  Future<SettingsEntity> get({required String id, bool isCache = false}) async {
    final doc = await root
        .doc(id)
        .get(!isCache ? null : const GetOptions(source: Source.cache));

    return SettingsEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<SettingsEntity> update({required SettingsEntity data}) async {
  ...
  Future<List<SettingsEntity>> load({
    ...
    return data.docs
        .map(
          (doc) => SettingsEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
  ...
  Future<void> loadNext({
    ...
    //       .map(
    //         (doc) =>
    //             SettingsEntity.fromJson(doc.data()! as Map<String, dynamic>),
    //       )
    //       .toList();
    //
  ...
  @override
  Stream<List<SettingsEntity>> get dataStream => controller.stream;
}
