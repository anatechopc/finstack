...
final class ReviewFirestoreService extends BaseFirestoreService<ReviewEntity> {
  ...
  @override
  String get collectionName => 'reviews';

  @override
  Future<ReviewEntity> add({required ReviewEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<ReviewEntity> delete({required ReviewEntity data}) async {
  ...
  @override
  Future<ReviewEntity> get({required String id, bool isCache = false}) async {
  ...
  @override
  Future<ReviewEntity> update({required ReviewEntity data}) async {
  ...
  Future<List<ReviewEntity>> load({
    ...
    return data.docs
        .map(
          (doc) => ReviewEntity.fromJson(doc.data()! as Map<String, dynamic>),
        )
        .toList();
  }
      ...
      controller.addStream(
        query.snapshots().map((snapshot) {
          return snapshot.docs
              .map(
                (doc) =>
                    ReviewEntity.fromJson(doc.data()! as Map<String, dynamic>),
              )
              .toList();
        }),
      ),
  ...
  @override
  Stream<List<ReviewEntity>> get dataStream => controller.stream;
}
