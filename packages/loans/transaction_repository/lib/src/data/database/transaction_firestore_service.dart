...
final class TransactionFirestoreService
    extends BaseFirestoreService<TransactionEntity> {
  ...
  @override
  Future<TransactionEntity> add({required TransactionEntity data}) async {
    final doc = root.doc();
  ...
  @override
  Future<TransactionEntity> delete({required TransactionEntity data}) async {
  ...
  @override
  Future<TransactionEntity> get(
      {required String id, bool isCache = false,}) async {
    final doc = await root
        .doc(id)
        .get(!isCache ? null : const GetOptions(source: Source.cache));

    return TransactionEntity.fromJson(doc.data()! as Map<String, dynamic>);
  }

  @override
  Future<TransactionEntity> update({required TransactionEntity data}) async {
  ...
  @override
  Future<List<TransactionEntity>> load({
      ...
      controller.addStream(
        query.snapshots().map((snapshot) {
          return snapshot.docs
              .map(
                (doc) => TransactionEntity.fromJson(
  ...
  @override
  Stream<List<TransactionEntity>> get dataStream => controller.stream;
}
