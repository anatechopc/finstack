...
**Design.** The three writers (`loanChanges`, `loanScheduleChanges`, `capitalCreated`) are adapter + core: `triggers/report_core.go` turns a parsed event into a `ReportPlan` (increments relative to `report_summary` + data items, pure, table-tested); `report_handlers.go` orchestrates transition check → product type → schedules → **claim the event id** (a 10-minute lease in a transaction on `{company}/report_events/{eventId}`, a sibling of `report_summary` so the client never streams claims) → **one atomic multi-path `Update` with `{".sv": {"increment": δ}}` that also writes the applied marker** (never released: a committed-but-errored write is recognised on retry, a failed one re-applies after the lease); `report_store.go` is the RTDB adapter. The old `applyToNodeValue` / `addReportDataItem` / `getProductType` / `getPathEnv` helpers are gone.

**Defects and outcomes.** D1: only status transitions count (`approved`/`bad_debt`/`completed` with `status != old`), redelivery caught by the claim. Only paid rows (`paid_on_time`/`paid_late`/`payment_submitted`) contribute principal to bad debt and settlement: the app also persists planned rows (the first schedule at approval has status `approved` and, for open-term loans, `principal_payment = amount`), which the first draft summed — the independent review's blocker. D2: server-value increments in one atomic update. D3: errors returned; malformed documents are logged and dropped (retrying cannot fix them); `--retry` enabled on the three triggers in `deploy_functions.sh`. D4/D7: rewritten. D5/D6: `completed` books the **remaining principal** `(amount + charges − deductions − upfront) − Σ principal − Σ extra` as collection, principal and returned capital, no interest (owner decision; the old Σ-of-schedules re-add double counted every payment). D8: `*firestoredata.DocumentEventData` / `*db.Client`; `go vet` clean. New: D13 (two items per event shared a key → second overwrote first; keys now suffixed), D14 (hard delete returned an error forever → nil), D15 (schedule query built `{env}_loan_schedules`, in production `_loan_schedules` → uses `GetCollectionPrefix()`).

**Proofs (RTDB emulator, `firebase emulators:start --only database --project demo-finstack`, tests with `FIREBASE_DATABASE_EMULATOR_HOST='localhost:9000?ns=demo-finstack' go test -tags emulator ./test/triggers/ -run Emulator`):** the six-node release plan under 50 concurrent writers lands 50/50 on every node, racy Get/Set 2/50 (also `go run ./cmd/race_demo`); 1 of 10 concurrent claims wins and 9 are deferred; the same loan event delivered twice books one release; claims are not under `report_summary`. Unit: 24 test functions in `test/triggers/report_*_test.go` (double delivery, 20 concurrent deliveries, same-status no-op, lease: a failed apply waits then applies once, a committed-but-errored apply is a no-op on retry, completed = 4200 with the planned rows present, settlement before any payment, overpayment floor, ISO-week year boundary, schedule/capital once) plus 4 internal parse tests in `triggers/report_parse_internal_test.go` (CI runs each module's tests).
